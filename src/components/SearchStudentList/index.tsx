...
const SearchStudentItem = memo(({ studentId: id, abbrevName, variant, avatarUrl }: SearchStudentItemProps) => {
    ...
    >
        ...
        >
            <IconButton
                radius='full'
                className='search-student-item__select-btn'
                color='blue'
                onClick={handleClick}
            >
            ...
            <Text className='search-student-item__name'>
                {abbrevName}
                {variant !== ''
                    && <Text
...
export default function SearchStudentList({ prompt, isFocus }: SearchStudentListProps) {
    ...
    return <ScrollArea
        ...
        {
            Object.entries((studentData satisfies StudentDataType)["students"]).filter(([, value]: [string, StudentItem]) => {
                if (prompt.trim() === '') {
                    if (window.innerWidth >= 640) {
                        return !isFocus
                    } else {
